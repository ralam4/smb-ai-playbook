...
export default function AgentPdfDownloadButton({ agent, className = '' }) {
  ...
  async function handleDownload() {
    ...
    try {
      const [{ pdf }, AgentPDFModule] = await Promise.all([
        import('@react-pdf/renderer'),
        import('./AgentPDF'),
      ])
      const AgentPDF = AgentPDFModule.default

      const blob = await pdf(<AgentPDF agent={agent} />).toBlob()
      const url = URL.createObjectURL(blob)
      const filename = `SMB-AI-Playbook-Agent-${slugifyFilename(agent.name)}.pdf`
